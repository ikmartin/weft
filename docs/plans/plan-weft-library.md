...
**[resolved, was 10.16] weft's HTTP client.** It was **ALPN**. Python's `urllib` never calls `set_alpn_protocols`, so its ClientHello carries no ALPN extension; every other common client sends one, and arXiv's edge answers a handshake without it with 406. Measured over six interleaved pairs against one OAI endpoint: the default context 3/6, an ALPN context 6/6, alternating on the same identifiers minutes apart — and on the original symptom, `arxiv.org/e-print/...`, 406 without and 200 with, seconds apart. `net.CONTEXT` now advertises `http/1.1` and both the metadata path and the downloader go through the opener that carries it. arXiv's OAI-PMH interface is reachable, which unblocks per-work version history and dates; `export.arxiv.org` stays the download host because arXiv asks automated clients to use it, not because anything is wrong with the other.
